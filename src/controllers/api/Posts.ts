...
class PostService {
  ...
  public async create (userId: number, text: string, imageBuffer?: Buffer, originalName?: string) {
    try {
      let mediaFileName: string | null = null;

      if (imageBuffer && originalName) {
        mediaFileName = await mediaServices.processAndSaveImage(imageBuffer, originalName);
      }
